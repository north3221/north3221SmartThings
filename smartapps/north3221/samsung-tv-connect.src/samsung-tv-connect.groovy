...
private tvAction(key,deviceNetworkId) {
    ...
    def port = deviceNetworkId.substring(9)
    def ipAddressLength = ipAddress.getBytes().size()

    //def macAddress = thetv?.value.mac.encodeAsBase64().toString()
    def macAddress = "C4576EBB7C68"
    def macAddressLength = macAddress.getBytes().size()

    //MY TV
    // [devicetype:04, mac:C4576EBB7C68, ip:C0A8000C, port:1DFC, ssdpPath:/smp_3_, ssdpUSN:uuid:03938700-003c-1000-bfb9-84a4662cf68d::urn:samsung.com:device:RemoteControlReceiver:1, ssdpTerm:urn:samsung.com:device:RemoteControlReceiver:1, hub:a39c4063-a476-48dc-8ed3-9a7352933512]
    ...
    if (key == "AUTHENTICATE") {
        sendHubCommand(new physicalgraph.device.HubAction(authenticationPacket, physicalgraph.device.Protocol.LAN, "${ipAddressHex}:${port}"))
    } else {
        ...
        //TEST COMMAND
        //REMOVE THIS
        //sendHubCommand(new physicalgraph.device.HubAction(authenticationPacket + actionPacket, physicalgraph.device.Protocol.LAN, "${ipAddressHex}:${port}"))
        //ADD THIS
        log.trace "IP Address     =" + ipAddress
        log.trace "IP Address HEX =" + ipAddressHex
        log.trace "Port           =" + port
        log.trace "Mac            =" + macAddress
        //g.trace  authPacket(" + authenticationPacket + ") actionPacket(" + actionPacket + ") LAN(" + physicalgraph.device.Protocol.LAN + ") IP_HEX(" + "${ipAddressHex}:1DFC)"
